// Combined Workout History JavaScript (Table + Card Views)
let allWorkouts = [];
...
function displayWorkouts(workouts) {
    if (currentView === 'table') {
        displayTableView(workouts);
    } else {
        displayCardView(workouts);
    }
}

function displayTableView(workouts) {
...
function displayCardView(workouts) {
    ...
    const workoutCards = workouts.map(workout => createWorkoutCard(workout)).join('');
    cardContainer.innerHTML = workoutCards;
}
...
function createWorkoutTableRow(workout) {
    ...
    const workoutSummary = createWorkoutSummary(workout.phases, workout.intensity, workout.type);
    
    const row = document.createElement('tr');
    row.className = `${intensityColors[workout.intensity] || intensityColors.normal} transition-colors`;
...
function updateViewButtons() {
    ...
    if (currentView === 'table') {
        tableBtn.className = 'bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors';
        cardBtn.className = 'bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors';
        tableView.classList.remove('hidden');
        cardView.classList.add('hidden');
    } else {
        tableBtn.className = 'bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-colors';
        cardBtn.className = 'bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors';
...
function showError(message) {
    const container = document.getElementById('workoutTableBody') || document.getElementById('cardView');
    container.innerHTML = `
